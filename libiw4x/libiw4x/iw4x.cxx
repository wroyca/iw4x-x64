...
namespace iw4x
{
  namespace
  {
    ...
    attach_console ()
    {
      ...
      // At this point, we've confirmed that neither standard stream is in
      // use, so we can now safely attach to the parent process's console.
      //
      if (AttachConsole (ATTACH_PARENT_PROCESS) == 0)
      {
        unsigned int error (GetLastError ());

        // `ERROR_ACCESS_DENIED` here usually means "you are already
        // attached". In that case, doing anything else would be redundant at
        // best and risky at worst, so we bail out.
        //
      ...
      // etc.) with the C FILE streams. Failed that, both could buffer
