  ...
  namespace mod
  {
    namespace
    {
      ...
      make_handler (const string& cmd)
      {
        ...
        handler->eventType = 0;
        handler->eventData.unconditionalScript = _strdup (cmd.c_str ());
      ...
      make_handler_set (const vector<string>& cmds)
      {
        ...
        handler_set->eventHandlerCount = static_cast<int>  (cmds.size ());
        handler_set->eventHandlers = new MenuEventHandler* [cmds.size ()];

        for (size_t i (0); i != cmds.size (); ++i)
          handler_set->eventHandlers[i] = make_handler (cmds[i]);
        ...
        ([] (const string& menu_name, const string& item_name, const vector<string>& cmds)
          {
            ...
            item.action = make_handler_set (cmds);
          },
