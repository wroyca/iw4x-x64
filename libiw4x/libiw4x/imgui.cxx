...
namespace iw4x
{
  ...
  imgui::imgui (renderer& r)
  {
    r.on_frame ().connect ([this] (IDirect3DDevice9* device)
    {
      ...
      // arrives.
      //
      static once_flag imgui_init_once_flag; call_once (imgui_init_once_flag,
                                                        [device] ()
      {
        HRESULT hr (0);
  ...
  void imgui::render_frame (IDirect3DDevice9* device)
  {
  ...
  LRESULT CALLBACK imgui::wnd_proc (HWND hwnd,
                                    UINT msg,
                                    WPARAM wparam,
                                    LPARAM lparam)
  {
    // Forward the message to ImGui first. If ImGui handles it, we're done.
    // Otherwise, we pass it through to the original window procedure to
    // maintain normal application behavior.
    //
    LRESULT imgui_result (ImGui_ImplWin32_WndProcHandler (hwnd, msg, wparam, lparam));
    if (imgui_result != 0)
      return imgui_result;

    // Fallback to the original window procedure if ImGui doesn't handle the
    // message.
    //
    if (original_wnd_proc_ != nullptr)
      return CallWindowProcW (original_wnd_proc_, hwnd, msg, wparam, lparam);

    // If no original procedure was stored, fall back to the default handler.
    // This should not normally happen if the hook was installed correctly.
    //
