...
namespace iw4x
{
  namespace
  {
    ...
    with_protection (void* addr, size_t s, DWORD p, F f)
    {
      DWORD o (0);
      if (VirtualProtect (addr, s, p, &o) == 0)
      {
      ...
      //
      if (VirtualProtect (addr, s, o, &o) == 0)
      {
        DWORD e (GetLastError ());
                                         ...
                                         [&vt] ()
    {
                                              ...
                                              IDirect3DDevice9** dv) -> HRESULT
      {
        ...
        if (SUCCEEDED (hr))
        {
          ...
          bool endscene (with_protection (&dvt [42],
                                          sizeof (void*),
                                          PAGE_EXECUTE_READWRITE,
                                          [&dvt] ()
          {
  ...
  renderer::notify (IDirect3DDevice9* d)
  {
    ...
    // Fire the signal to notify all connected slots. If any slot throws,
    // we terminate to avoid undefined behavior.
    //
    try
    {
      frame_signal_ (d);
